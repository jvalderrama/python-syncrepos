...
function usage {
  echo "Usage: $0 [OPTION]..."
  echo "Run reposynch's test suite(s)"
  echo ""
...
function run_tests {
  ...
  if [ $debug -eq 1 ]; then
    if [ "$testropts" = "" ] && [ "$testrargs" = "" ]; then
      ...
      # provided.
      testrargs="discover ./reposynch/tests"
    fi
  ...
  if [ setup.cfg -nt reposynch.egg-info/entry_points.txt ]
  then
  ...
  if [ $coverage -eq 1 ]; then
    ...
    ${wrapper} coverage html --include='reposynch/*' --omit='reposynch/openstack/common/*' -d covhtml -i
  fi
